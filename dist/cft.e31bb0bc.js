...
var url = 'http://localhost:3001/files';
var main = document.querySelector('main'); // main tag
...
var inputDate = Array.from(document.querySelectorAll('.input_date'));
var arrData = []; // for data 

inputDate.map(function (input) {
...
loadServerData(); // function request data

function loadServerData() {
  fetch(url).then(function (response) {
    return response.json();
  }).then(function (json) {
    arrData = _toConsumableArray(json);
    renderData(arrData);
  });
}

function filterData(firstDay, lastDay, type) {
  var copyData = _toConsumableArray(arrData);

  if (firstDay) {
    copyData = copyData.filter(function (obj) {
      return new Date(firstDay) <= new Date(obj.load);
    });
  }

  if (lastDay) {
    copyData = copyData.filter(function (obj) {
      return new Date(lastDay) >= new Date(obj.load);
    });
  }
...
function renderData(arr) {
  ...
  var tmp = document.querySelector('#tmp'),
      ...
      dateContainer = container.querySelector('.date_container'); // loop fill render content 

  var _loop = function _loop(index) {
    var element = arr[index]; // element content fill   

    typeFile.textContent = element.type;
    fileName.textContent = element.filename; // console.log();

    size.textContent = returnSize(element.filesize);
    userName.textContent = element.name;
    var timeStore = new Date(element.load);
    dateLoad.textContent = "".concat(timeStore.toLocaleDateString('ru'));
    timeStore.setFullYear(timeStore.getFullYear() + 1);
    dateContainer.textContent = "".concat(timeStore.toLocaleDateString('ru'));
    var clone = document.importNode(tmp.content, true);
    var deleteBtn = clone.querySelector('.delete');
    var contDel = clone.querySelector('.container'); // delete element on node and data
...
function returnSize(size) {
  ...
  if (count === 0) {
    return "".concat(size, " bt");
  } else if (count === 1) {
    return "".concat(size, " kb");
  } else if (count === 2) {
    return "".concat(size, " mb");
  } else if (count === 3) {
    return "".concat(size, " gb");
  } else {
    return "".concat(size, " tb");
  }
...
function postFile() {
  var fileLoad = file.files[0]; // file in input

  var typeStr = /(?<=\.)\w+$/i; // regex for search type

  var startStr = /^.+(?=\.)/i; // regex for search name
...
},{"./src/scss/main.scss":"src/scss/main.scss"}],"node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
//# sourceMappingURL=/cft.e31bb0bc.js.map